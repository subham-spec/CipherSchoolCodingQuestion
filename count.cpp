#include <vector>
#include <iostream>
using namespace std;
    int f(int i, int sum, vector<int>& nums, vector<vector<int>>& dp){
        if(sum==0)
            return 1;
        if(i==0){
            if(nums[i]==sum)
                return 1;
            return 0;
        }
        if(dp[i][sum] != -1)
            return dp[i][sum];
        int nt = f(i-1, sum, nums, dp);
        int t=0;
        if(nums[i]<=sum)
            t=f(i-1, sum-nums[i], nums, dp);
        
        return dp[i][sum]=(t+nt);
    }
    // main function isko call krega array and target
    int countSubset(vector<int>& array) {
        vector<int> v;
        for(int i=1; i<array.size(); i++)
            v[i-1] = array[i];

        vector<vector<int>>dp(v.size(), vector<int>(array[0]+1, -1));
        
        return f(v.size()-1, array[0], v, dp);
    }
int main(){
    vector<int> nums{10,2,3,5,5};
    cout<<"Is partition possible : \n"<<countSubset(nums, sum)<<endl;
}